...
// Upon not finding an order, returns a nullptr rather than return an error, so
// it's easier to use in other functions for checking if an order exists.
...
const std::map<int, std::shared_ptr<Order>>& OrderRepository::getOrders()
    const {
  return orders;
}
...
std::map<int, std::shared_ptr<Order>> OrderRepository::getCompletedOrders()
    const {
...
float OrderRepository::calculateProfits(
    const std::map<int, std::shared_ptr<Order>>& completed_orders) const {
  float profits = 0.0f;
...
// checks that input order still has a default orderid, if so orderid is set to
// next_id (which is then incremented) and order is added to repository if the
// above is not true, throws an error
