...
//switchOrderStatus, consumeMaterial, submit ReceptReport, submit PhotoReport, listMaterials
	...
	auto admin = std::make_shared<Administrator>(order_repository, "Binkle Bonkler", material_repository, receptreport_repository, photoreport_repository);
	...
	auto receptionist = std::make_shared<Receptionist>(order_repository, "Schmongler", clock, receptreport_repository);
	...
	auto badreceptionist = std::make_shared<Receptionist>(order_repository, "Terrible Receptionist", clock, receptreport_repository);
	...
	std::map<int, std::shared_ptr<Employee>> employees =  employee_repository->getEmployees();
	...
	auto photographer = std::make_shared<Photographer>(order_repository, "Kababoomgler", clock, material_repository, photoreport_repository);
	...
	//std::cout << "Total revenue of completed orders: " << receptreport_repository->findReport(receptreportid)->total_revenue << "\n";

	////std::cout << "Consumed materials: " << "\n";
	//for (const auto& [material, quantity] : photoreport_repository->findReport(photoreportid)->consumed_materials) {
